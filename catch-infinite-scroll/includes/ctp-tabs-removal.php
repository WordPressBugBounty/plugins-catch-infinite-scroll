...
if (! function_exists('ctp_switch')) {
	...
	function ctp_switch()
	{
		...
		if (! isset($_POST['security'], $_POST['value'], $_POST['option_name'])) {
			wp_die(esc_html__('Invalid request.', 'catch-infinite-scroll'));
		}

		// Sanitize & verify nonce
		$nonce = sanitize_text_field(wp_unslash($_POST['security']));
		if (! wp_verify_nonce($nonce, 'ctp_tabs_nonce')) {
			wp_die(esc_html__('Unauthorized access!', 'catch-infinite-scroll'));
		}

		// Capability check
		if (! current_user_can('manage_options')) {
			wp_die(esc_html__('Permission denied!', 'catch-infinite-scroll'));
		}
		...
		if (update_option('ctp_options', $option_value)) {
			...
			esc_html_e('Connection Error. Please try again.', 'catch-infinite-scroll');
		}
